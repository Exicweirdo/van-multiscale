...
from system import ising1D
from script.args import args
from model.made1d import MADESimple1d, MADEnew1d
...
def main():
    ...
    if args.net == 'made':
        net = MADESimple1d(n=args.L, **vars(args))
    elif args.net == 'madenew':
        net = MADEnew1d(n=args.L, **vars(args))
    net.to(args.device)
    my_log("{}\n".format(net))
    ...
    for step in range(last_step + 1, args.max_step + 1):
        ...
        with torch.no_grad():
            energy = ising1D.energy(sample, args.ham, args.boundary)
        ...
        if args.print_step and step % args.print_step == 0:
            free_energy_mean = loss.mean() / args.beta / args.L
            free_energy_std = loss.std() / args.beta / args.L
            entropy_mean = -log_prob.mean() / args.L
            energy_mean = energy.mean() / args.L
            ...
            my_log(
                "step = {}, F = {:.8g}, F_std = {:.8g}, S = {:.8g}, E = {:.8g}, M = {:.8g}, Q = {:.8g}, lr = {:.3g}, beta = {:.8g}, sample_time = {:.3f}, train_time = {:.3f}, used_time = {:.3f}".format(
                    step,
            ...
            if args.save_sample:
                ...
                torch.save(
                    state, "{}_save/{}.sample".format(args.out_filename, step))
        ...
        if args.out_filename and args.save_step and step % args.save_step == 0:
            ...
            torch.save(
                state, "{}_save/{}.state".format(args.out_filename, step))

        if (args.out_filename and args.visual_step
                and step % args.visual_step == 0 and step != 0):
            sample_tensor = sample
            grid = make_grid_ising(sample_tensor, dimension=1)
            ...
            if args.print_sample:
                ...
                x_hat_std = np.std(x_hat_np, axis=0).reshape([args.L])
                ...
                x_hat_corr = x_hat_corr.reshape([args.L])
