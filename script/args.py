...
group.add_argument(
    "--ham", type=str, default="afm", choices=["afm", "fm"], help="Hamiltonian model"
)

group.add_argument(
    "--lattice", type=str, default="sqr", choices=["sqr", "tri"], help="lattice shape"
)
...
group.add_argument("--L", type=int, default=4, help="number of sites on each edge of the lattice")
...
group.add_argument(
    "--net", type=str, default="made", choices=["made", "madenew", "madeconst"], help="network type"
)
group.add_argument(
    "--model", type=str, default="made", choices=["made", "parallel_made", "half_parallel_made", "tri_parallel_made"], help="model name"
)
...
group.add_argument(
    "--dtype", type=str, default="float32", choices=["float32", "float64"], help="dtype"
)
...
group.add_argument(
    "--final_conv", action="store_true", help="add an additional conv layer before sigmoid"
)
group.add_argument(
    "--epsilon", type=float, default=1e-7, help="small number to avoid 0 in division and log"
)

group = parser.add_argument_group("optimizer parameters")
group.add_argument("--seed", type=int, default=0, help="random seed, 0 for randomized")

group.add_argument("--glass_seed", type=int, default=0, help="graph correlation strength random seed, 0 for randomized")
group.add_argument(
    "--optimizer",
    type=str,
    default="adam",
...
group.add_argument("--max_step", type=int, default=10**4, help="maximum number of steps")
...
group.add_argument(
    "--clip_grad", type=float, default=0, help="global norm to clip gradients, 0 for disabled"
)
...
group.add_argument(
    "--print_step", type=int, default=1, help="number of steps to print log, 0 for disabled"
)
group.add_argument(
    "--save_step",
    type=int,
...
group.add_argument(
    "--checkpoint_file",
    type=str,
    help="checkpoint file to load network weights"
)
...
group.add_argument(
    ...
    help="type of MC update strategy",
)
...
group.add_argument(
    ...
    type=int,
    default=1,
    help="number of steps to record data, 0 for disabled",
)

group = parser.add_argument_group("ST parameters")
group.add_argument("--beta_start", type=float, default=1, help="beta low")
group.add_argument("--beta_end", type=float, default=3, help="beta high")
group.add_argument("--beta_step", type=float, default=0.1, help="beta step")
