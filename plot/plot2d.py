    ...
    def f(x):
        return np.log(
            ...
            * (
                1
                + np.sqrt(
                    1 - (2 * np.sinh(2 * beta) / np.cosh(2 * beta) ** 2) ** 2 * np.sin(x) ** 2
                )
            )
        )
...
def gamma_r(beta, r, n):
    ...
    result = np.where(
        r > 0,
        np.arccosh(coshgam),
        np.where(r == 0, -(np.arctanh(np.exp(-beta * 2)) - beta) * 2, np.nan),
    )
...
def F_periodic(beta, n):
    ...
    logZ += (
        np.log(
            np.prod(2 * np.cosh(n / 2 * gamma_r(beta, rlist * 2, n)) / np.exp(n / 2))
            + np.prod(2 * np.cosh(n / 2 * gamma_r(beta, rlist * 2 + 1, n)) / np.exp(n / 2))
            + np.prod(2 * np.sinh(n / 2 * gamma_r(beta, rlist * 2, n)) / np.exp(n / 2))
            + np.prod(2 * np.sinh(n / 2 * gamma_r(beta, rlist * 2 + 1, n)) / np.exp(n / 2))
        )
        / n**2
        + 1 / 2
    )
...
def plot_F(F_dict: dict, **kwargs):
    ...
    for modelconfig in np.sort(list(F_dict.keys())):
        print(modelconfig)
        ...
        Fs_dif = np.array([F_dict[modelconfig][beta][0] for beta in betas]) - np.array(
            [F_periodic(beta, L) for beta in betas]
        )
        Fs_dif = np.abs(Fs_dif) / np.abs(np.array([F_periodic(beta, L) for beta in betas]))
        F_stds = np.array([F_dict[modelconfig][beta][1] for beta in betas])
        dif_ax.plot(betas, Fs_dif, label=modelconfig, **kwargs)
    ax.legend()
    dif_ax.legend()
...
def get_F_dict(root_dir: str):
    ...
    for systemname in os.listdir(root_dir):
        ...
        print(beta)
        for modelconfig in os.listdir(os.path.join(root_dir, systemname)):
            if not F_dict.get(modelconfig):
                F_dict[modelconfig] = {}
            logfile = os.path.join(root_dir, systemname, modelconfig, "out.log")
            ...
            with open(logfile, "r") as f:
                ...
                F_std = float(re.search(r".*F_std = ([\d\.\w-]+),", lastline).group(1))
                F_dict[modelconfig][beta] = (F, F_std)
    return F_dict


if __name__ == "__main__":
    F_dict_row = get_F_dict("./out2d")

    fig_F, ax_F, dif_Fig, dif_ax = plot_F(F_dict_row, marker="o", alpha=0.5)
    betas = np.linspace(0.2, 1.0, 100)
    F_anals = np.array([F_periodic(beta, L) for beta in betas])
    ax_F.plot(betas, F_anals, label="exact")
    ax_F.set_xlim(0.38, 1.02)
    ax_F.set_ylim(-2.22, -1.98)
    ax_F.legend()
    fig_F.savefig("F2d.png")
    dif_Fig.savefig("F2d_dif.png")
