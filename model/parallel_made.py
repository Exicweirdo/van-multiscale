...
class ParallelMade(nn.Module):
    def __init__(self, **kwargs):
        super(ParallelMade, self).__init__()
        self.L = kwargs["L"]
        self.n = self.L**2
    ...
    def forward(self, x: torch.Tensor):
        sample_fold = ParallelMade.fold(x, self.L)
        x_hat = self.forward_onfolded(sample_fold)
        x_hat = ParallelMade.unfold(x_hat, self.L)
    ...
    def sample(self, batch_size):
        ...
        sample = ParallelMade.unfold(sample_fold, self.L)
        x_hat = ParallelMade.unfold(x_hat_fold, self.L)
        return sample, x_hat
    ...
    def partial_sample(self, batch_size, sample_init=None, start=0):
        ...
        if sample_init is not None:
            sample_init_fold = ParallelMade.fold(sample_init, self.L)
    ...
    def log_prob(self, sample):
        with torch.no_grad():
            sample_fold = ParallelMade.fold(sample, self.L)
